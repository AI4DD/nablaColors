    ...
    def load_checkpoint(
        ...
        **passthrough_args,
        ...
        if bexists:
            ...
            with open(self.args.chemprop_weight_path, "rb") as f:
                state_chempr = torch.load(
                    f, map_location=torch.device("cpu"), weights_only=False
                )

            append_keys = [
                key for key in state_chempr["state_dict"] if "encoder.1" in key
            ]
            new_keys = [key for key in state_chempr["state_dict"] if "encoder.0" in key]

            for i in range(len(append_keys)):
                app_key = append_keys[i]
                new_key = "chemprop_" + new_keys[i]
                ema_state["params"][new_key] = state_chempr["state_dict"][app_key]

            # for i in range(len(append_keys)):
            #     app_key = append_keys[i]
            #     new_key = 'chemprop_' + new_keys[i]
            #     state["model"][new_key] = state_chempr["state_dict"][app_key]
            if self.args.head_pretrain:
                logger.info("")
                energy_layers_list = []                 # modif block
                n = 768 #+ 482
                for key in ema_state["params"].keys():   # modif
                    if 'energy' in key:
                        energy_layers_list.append(key)
                        if key == 'energy_head.linear_out.bias':
                            ema_state["params"][key] = torch.randn((1))
                        elif key == 'energy_head.linear_out.weight':
                            ema_state["params"][key] = torch.randn((1, n))
                        elif 'bias' in key or 'layer' in key:
                            ema_state["params"][key] = torch.randn((n))
                        elif 'linear' in key:
                            ema_state["params"][key] = torch.randn((n, n))

            # energy_layers_list = []                 # modif block
            # for key in state["model"].keys():   # modif
            #     if 'energy' in key:
            #         energy_layers_list.append(key)
            #         if key == 'energy_head.linear_out.bias':
            #             state["model"][key] = torch.randn((1))
            #         elif key == 'energy_head.linear_out.weight':
            #             state["model"][key] = torch.randn((1, n))
            #         elif 'bias' in key or 'layer' in key:
            #             state["model"][key] = torch.randn((n))
            #         elif 'linear' in key:
            #             state["model"][key] = torch.randn((n, n))

            # load model parameters
            # --- Begin patch: allow finetuning from checkpoints with different head sizes ---
            # When we load a pretrained model whose output head has a different
            # dimensionality (e.g. pretrained on 1-target energy, finetune on 3 targets),
            # some parameters will have incompatible shapes.  Remove those keys before
            # calling `load_state_dict` so that we can still reuse the rest of the
            # checkpoint weights.

            def _filter_mismatched_keys(target_model, state_dict):
                """Return a new state dict keeping only tensors whose shapes match
                the current model.  We also log the skipped keys for transparency."""
                ...
                if mismatched:
                    logger.info(
                        f"Skipped {len(mismatched)} parameter(s) due to size mismatch: {mismatched}"
                    )
                return filtered

            if self.args.load_from_ema:
                ema_state["params"] = _filter_mismatched_keys(
                    self.model, ema_state["params"]
                )
            else:
                state["model"] = _filter_mismatched_keys(self.model, state["model"])
            # --- End patch ---
            try:
                if self.args.load_from_ema:
                ...
                # If training in FP16, ensure newly-loaded parameters are cast to half.
                if self.args.fp16 and not self.args.bf16:
                    logger.info(
                        "Casting model parameters to FP16 after checkpoint load"
                    )
                    self.model.half()
                    if self.ema is not None:
                        self.ema.half()
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            assert last_optim["loss_name"] == self.get_loss().__class__.__name__, (
            ...
            if self.ema is not None:
                with torch.autograd.profiler.record_function("ema"):
                    if self.args.fp16 or self.args.bf16:
                        ...
                        self.ema.update(
                            self.model.named_parameters(), is_flattened=False
                        )
