...
if [ -z "${pretrained_model:-}" ]; then
    pretrained_model="../results/checkpoints_unimol/exp_${exp_name}/run_bs_64_head_pretrain_fold_${fold_name}/-ema${ema_decay}/checkpoint_best_exp${exp_name}_runbs_64_head_pretrain_fold_${fold_name}.pt"
fi
...
command="torchrun --nproc_per_node=$n_gpu --nnodes=$OMPI_COMM_WORLD_SIZE  --node_rank=$OMPI_COMM_WORLD_RANK  --master_addr=$MASTER_IP --master_port=$MASTER_PORT \
