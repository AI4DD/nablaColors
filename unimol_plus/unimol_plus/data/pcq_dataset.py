...
class PCQDataset(BaseWrapperDataset):
    ...
    def __getitem_cached__(self, epoch: int, idx: int):
        with data_utils.numpy_seed(self.seed, epoch, idx):
            ...
            feat["id"] = data["id"]
            # feat["embedding"] = data["embedding"]  #modif
            feat["solvent_smi"] = data["solvent_smi"]  #modif
            return feat
    ...
    def collater(self, items):                                   # FIXME make own fluor dataset class
        target = np.stack([np.array(x["target"]) for x in items])
        
        id = np.stack([int(x["id"]) for x in items]).astype(np.int64)   
        # solv_emb = np.stack([x["embedding"] for x in items])
        pad_fns = {
            "atom_feat": data_utils.pad_1d_feat,
            "atom_mask": data_utils.pad_1d,
            "edge_feat": data_utils.pad_2d_feat,
        ...
        batched_data["target"] = torch.from_numpy(target).float()
        batched_data["id"] = torch.from_numpy(id).long()
        # batched_data["embedding"] = torch.from_numpy(solv_emb).float()  # modif
        batched_data["solvent_smi"] = [x["solvent_smi"] for x in items]  # modif
        return batched_data
