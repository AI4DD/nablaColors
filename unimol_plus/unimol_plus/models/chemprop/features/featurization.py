...
def get_bond_fdim(
    ...
    return (
        (not overwrite_default_bond) * BOND_FDIM
        + EXTRA_BOND_FDIM
        + (not atom_messages)
    ...
    * :code:`overwrite_default_bond_features`: A boolean to overwrite default bond descriptors.
    ...
    def __init__(
        ...
        if isinstance(mol, str):
            if self.is_reaction:
                mol = (
                    make_mol(mol.split(">")[0], self.is_explicit_h),
                    make_mol(mol.split(">")[-1], self.is_explicit_h),
                )
            else:
